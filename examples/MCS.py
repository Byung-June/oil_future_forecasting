...
def MCS_table(data_list):
    df_prev = pd.DataFrame()
    for data in data_list:
        df = pd.read_csv(data, index_col=0)
        if ('arima' in data) or ('lag' in data):
            if 'arima' in data:
                col_name = 'arima_' + data.split('_')[-3] + '_' + data.split('_')[-2] + '_'  + data.split('_')[-1].split('.csv')[0]
            else:
                col_name = 'lag'
            df = df.set_index('0')
            df.index = df.index.rename('date')
            df[col_name] = (df['1'] - df['2']) ** 2

        else:
            temp = data.split('\\')[-1].split(('windows'))
            col_name = temp[0].split('res_')[-1] + temp[1].split('features_')[-1].split('_')[0]
            # col_name = data.split('\\')[-1].split('_')[1] + '_' + data.split('\\')[-1].split('_')[9]
            df[col_name] = (df['y_pred'] - df['y_test']) ** 2

        if len(df_prev) == 0:
            df_prev = df[[col_name]]
            continue
        df_prev = pd.merge(df_prev, df[[col_name]], left_index=True, right_index=True, how='inner').dropna()
    ...
    try:
        mcs = ModelConfidenceSet(df_prev, 0.1, 3, 1000).run()
        included = mcs.included
        pvalues = mcs.pvalues
        pvalues.to_csv('result_table//%s.csv' % file_name)
        print(included)
        print(pvalues)
...
if __name__ == "__main__":
    ...
    path = 'D:\Dropbox/6_git_repository\oil_future_forecasting/results/logvol_*'
    data_list = glob.glob(path)
    for data_sub_list in data_list:
        data_sub_list = glob.glob(data_sub_list + '/*.csv')
        included, pvalues = MCS_table(data_sub_list)

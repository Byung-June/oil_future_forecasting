...
def rolling_sample(data_, window_num, sample_num):
    ...
    for time in np.arange(window_num + sample_num - 2, total_sample_num):
        ...
        # print('train list', data_x_train)
        # print('test list', data_x_test)
        # print('train y', data_y_train)
        # print('test y', data_y_test)
    ...
    return data_x_train_time, data_x_test_time, data_y_train_time, data_y_test_time


if __name__=='__main__':

    idx = pd.date_range('2018-01-01', periods=7, freq='D')
    ts = pd.DataFrame(range(len(idx)), index=idx, columns=['value'])
    ts2 = pd.DataFrame(range(1, len(idx) + 1), index=idx, columns=['value'])
    ts3 = pd.DataFrame(range(2, len(idx) + 2), index=idx, columns=['value'])
    ts = pd.merge(ts, ts2, left_index=True, right_index=True)
    ts = pd.merge(ts, ts3, left_index=True, right_index=True)
    print(ts)
    print(rolling_sample(ts, 2, 4))
    print('done!')
